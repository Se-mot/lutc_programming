...
import cgi, html     # cgi.test() выведет поля ввода
import os
import shelve
import sys

shelvename = 'class-shelve'  # файлы хранилища находятся в текущем каталоге

fieldnames = ('name', 'age', 'job', 'pay',)

form = cgi.FieldStorage()  # парсинг данных формы
print('Content-type: text/html')  # заголовок+пустая строка для ответа
sys.path.insert(0, os.getcwd())  # благодаря этому модуль pickle и сам сценарий
# будут способны импортировать модуль person
...
def updateRecord(db, form):
    if not 'key' in form:
        ...
        if key in db:
            ...
            record = Person(name='?', age='?')  # eval: сероки должны быть заключены в кавычки
...
if action == 'Fetch':
    ...
    fields['key'] = 'Missing or invalid action!'  # кнопки отправления форм
