...
ей функцию, для которой стандартные потоки ввода и вывода будут связаны
...
class Input:  # имитирует входной файл
    ...
    def read(self, size=None):  # необязательный аргумент
        if size is None:  # прочитать N байт или все
...
def redirect(function, pargs, kargs, input):  # перенаправляет stdin/stdout
